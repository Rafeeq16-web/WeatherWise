...
const DailyWeather = (props: any) => {
  // State to store daily weather data
  ...
  // Effect to update weather codes when daily data changes
  useEffect(() => {
    if (dailyData.time.length > 0) {
      const codes = dailyData.weather_code;
      setWeatherCodes(codes);
    }
  }, [dailyData]);

  // Function to fetch weather data for each code
  const getWeatherData = (code: any) => {
    dispatch({ type: code });
  };

  // Effect to fetch daily weather data
  useEffect(() => {
  ...
  // Function to fetch daily weather data from API
  const fetchData = async () => {
    try {
      ...
      setDailyData(result.data.daily); // Update daily weather data state
    } catch (error) {
